...
%  Grupo 01. Cifras verificadas mediante la ejecución completa del notebook.
...
\newcommand{\legajoDietrich}{370913}
...
% =====================================================================
\section*{Resumen}
En este trabajo se estudia la reducción de dimensionalidad de radiografías de tórax mediante Análisis de
Componentes Principales (PCA) y su efecto sobre la clasificación de imágenes normales y con neumonía. Se
implementa PCA mediante la descomposición en valores singulares y se evalúa una regresión logística al
conservar distintas cantidades de componentes. Los resultados obtenidos muestran que representaciones de
baja dimensión pueden alcanzar un desempeño comparable al de las imágenes completas. Luego se analiza,
mediante simulaciones Monte Carlo, la robustez del sistema frente a rotaciones aleatorias de las imágenes
de prueba, y se observa que el desempeño baja a medida que aumenta la probabilidad de rotación. Finalmente,
se justifica la estimación de la probabilidad de pérdida mediante la Ley de los Grandes Números y se
discuten los alcances y las limitaciones del análisis.
...
En la segunda parte analizamos qué tan robusto es este sistema si algunas imágenes llegan rotadas $180^\circ$.
...
\paragraph{Clasificador y métrica.} Como indica la consigna, usamos la regresión logística de
\emph{scikit-learn} solo como herramienta para medir cuánta información conserva cada
representación, sin analizar su funcionamiento interno~\cite{mahmood}. Mantuvimos su configuración por
defecto (regularización $L_2$, $C=1$, tolerancia $10^{-4}$) y modificamos únicamente el número máximo de
iteraciones, por los motivos que se explican a continuación.

\paragraph{Convergencia del ajuste.} Empezamos con el máximo de 2000 iteraciones del ejemplo de la
consigna. Con ese valor, 21 de los 300 ajustes del barrido lo agotan sin alcanzar la tolerancia y
\emph{scikit-learn} emite la advertencia correspondiente (\texttt{ConvergenceWarning}): son los $K$ entre
204 y 229, salvo 205, 206, 212, 226 y 227. Fuimos probando entonces con límites cada vez mayores y la
cantidad de ajustes afectados fue bajando, pero siempre quedaron siete sin converger: los $K$ entre 218 y
224 no alcanzan la tolerancia por más que se aumente el límite, incluso con valores mucho mayores que los
que usamos. No identificamos la causa de ese comportamiento, y resolverlo implicaría usar otras
herramientas, algo que queda fuera del alcance de este trabajo.

Fijamos entonces $\texttt{max\_iter}=20000$ para todo el barrido, con el que solo quedan esos siete casos,
y dejamos anotado cuáles son. Para medir su efecto los comparamos con ajustes equivalentes que sí
llegan a converger: cuatro dan exactamente el mismo \acc{} y los otros tres difieren en 2, 2 y 6 imágenes
sobre 468 ($K=221$, $222$ y $223$), es decir como máximo 0.013 de \acc{}. El ajuste con las imágenes
completas converge en 89 iteraciones y el del Ejercicio 2, con $K=2$, en 11, de modo que ninguno de los dos
depende de este límite y el segundo conserva el valor de 2000 del ejemplo. Además, ningún $K$ de los que
reportamos en las tablas, en el máximo del barrido o en las conclusiones (Sección~\ref{sec:concl}) cae
entre 218 y 224. La falta de convergencia queda así limitada a un tramo corto de la curva de la
Figura~\ref{fig:acck} y no cambia los resultados del trabajo de forma importante.

Entrenamos con train y medimos el \acc{} sobre las 468 imágenes de test. Cada acierto
modifica el \acc{} en $1/468\approx0.002$, de modo que una diferencia de 0.01 equivale a unas 5 imágenes.
El barrido de $K$ sobre test es descriptivo, como solicita la consigna: el máximo observado no es una
estimación independiente del desempeño de un $K$ seleccionado. Para elegir $K$ para un uso posterior
habría que validar dentro del conjunto de entrenamiento y reservar test para la evaluación final.

\paragraph{Organización del código.} El código se presenta en un único cuaderno de Jupyter, organizado
en dos partes correspondientes a los ejercicios 1 y 2. Se definieron funciones para cargar y vectorizar
las imágenes, ajustar PCA, proyectar los datos y entrenar y evaluar el clasificador. En la segunda parte
se realiza la simulación Monte Carlo. El cuaderno debe ejecutarse en orden desde el inicio y guarda las
figuras que se incluyen en este informe; los valores numéricos que citamos son los que imprime en sus
salidas. La descomposición en valores singulares y las operaciones con matrices se resuelven con
NumPy.
...
un \acc{} de \textbf{0.818} (383 aciertos de 468), que usamos como línea de base. El modelo ajusta
un peso por píxel y un intercepto con 2428 imágenes de entrenamiento. Esta alta dimensión motiva estudiar
representaciones más pequeñas, pero la relación entre número de parámetros y muestras no demuestra por
sí sola la existencia de sobreajuste, especialmente cuando se utiliza regularización.
...
todo el rango, por lo que esta coordenada sola tiene una capacidad de discriminación limitada. Sobre la componente 2 (eje
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.51\textwidth}\vspace{0pt}
    \centering
    \includegraphics[width=\linewidth]{accuracy_vs_k.png}
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccccccccc}
    ...
    \emph{Accuracy}    & 0.596 & 0.803 & 0.829 & \textbf{0.857} & 0.808 & 0.825 & 0.829 & 0.831 & 0.812 & 0.784 \\
    \bottomrule
  \end{tabular}
\end{table}

Con $K=1$ el \acc{} es 0.596: esta componente discrimina menos que el par formado por las dos primeras.
Con $K=2$ sube a 0.803 y con $K=3$ supera la línea de base: 0.829 frente a 0.818. El máximo observado entre
1 y 300 componentes corresponde a $K=5$ y es 0.857 (401 aciertos). Así, en este conjunto de
prueba unas pocas coordenadas permiten obtener un desempeño superior al de la representación completa.
La curva no es monótona: para valores intermedios oscila alrededor de la referencia y luego desciende,
hasta 0.784 con $K=300$. Estas variaciones describen los ajustes evaluados y no prueban que las
componentes de baja varianza sean ruido ni que la caída se deba exclusivamente al sobreajuste. Además, en
el tramo $218\le K\le224$ se mezclan con el efecto de los siete ajustes que no llegan a converger, por lo que
ese tramo de la curva debe leerse con esa aclaración.

La mejora con pocas componentes coincide con la idea de que la reducción de dimensión limita el ajuste
de variaciones poco útiles para esta clasificación, aunque para comprobarlo harían falta más evaluaciones.
...
negativo solo refleja la elección arbitraria del signo. Esta componente tiene una capacidad de separación
individual limitada. La \textbf{PC2} (10.1\,\%) presenta signos opuestos en regiones centrales y periféricas
del tórax. El patrón visual sugiere que representa contrastes de intensidad entre esas zonas, que coinciden
con diferencias observables entre las clases. Sin embargo, no se midió opacidad pulmonar ni se aislaron
factores como exposición, tamaño o encuadre; por eso no interpretamos esta componente como una medida
clínica específica. Las \textbf{PC3 a PC6} muestran otros contrastes espaciales y menor separación
individual. Esto no impide que aporten información cuando se combinan con otras coordenadas.
...
varianza. La varianza conservada y la capacidad de clasificación miden aspectos diferentes.
...
coordenada. La Tabla~\ref{tab:fisher} indica que PC2 tiene la mayor separación individual entre las seis componentes
analizadas: su $F$ es más de seis veces el de PC1 y unas veinte veces el de PC3 a PC6, y alcanza un
\acc{} de 0.784 por sí sola. PC1 también aporta cierta separación, aunque menor. PCA ordena las
direcciones por varianza sin utilizar las etiquetas, por lo que la dirección de mayor varianza no tiene
por qué ser la más útil para clasificar.
...
\begin{itemize}
  \item \textbf{Normalización.} Dividimos los píxeles por 255, para que queden entre 0 y 1, porque así el
        ajuste de la regresión logística converge con más facilidad. La escala uniforme conserva las direcciones de PCA y reescala sus coordenadas, pero no garantiza
        que la regresión logística regularizada produzca las mismas predicciones con $C$ fijo. En esta
        ejecución, el \acc{} sin rotaciones coincidió con el del ejercicio 1 para $K=2$.
  \item \textbf{Rotación y sorteo.} Para rotar una imagen, reacomodamos su vector como matriz de
        $128\times128$, la giramos dos veces $90^\circ$ y la volvemos a convertir en vector. Esto equivale a
  ...
  \item \textbf{Cantidad de simulaciones.} Elegimos $N_{MC}=2000$ a partir de la desigualdad de Chebyshev
        (Sección~\ref{sec:ej3}). Para cada $p$ fijado, el desvío estándar del estimador de probabilidad es a lo
        sumo 0.0112 y la probabilidad de equivocarse en 0.05 o más es como máximo 0.05.
  \item \textbf{Valores de $p$.} Para los incisos (b) y (c) usamos 18 valores, de 0.05 a 0.90 con paso
        0.05, para obtener curvas bien definidas. Para (a) usamos 0.1, 0.3, 0.5, 0.7 y 0.9, además de $p=0$ como referencia.
        En (d) mostramos los histogramas de esos mismos cinco valores, que reutilizan las simulaciones
        de (b) y (c).
\end{itemize}
...
Si $R(x)$ es la imagen $x$ rotada, por linealidad se cumple
$R(x)-\mu=R(x-\mu)+(R(\mu)-\mu)$. El segundo término es común a todas las imágenes y su proyección vale
$(-1.7,-3.9)$ en las coordenadas normalizadas. El primero depende de cada imagen y se proyecta sobre
direcciones aprendidas con radiografías sin rotar. En la ejecución observamos que el desvío de PC2 pasa de
5.6 a 2.8 y que las medias de las dos clases rotadas quedan cerca de $-4.2$: disminuye la separación de
sus centros sobre ese eje. PC1, asociada al brillo global, se conserva en mayor medida (la correlación
entre sus coordenadas antes y después de rotar es 0.99). Esta descomposición ayuda a interpretar la
franja observada.

Con todo test rotado, el 97.4\,\% de las imágenes se clasifica como neumonía, frente al
57.3\,\% sin rotar. La proporción de aciertos en Neumonía pasa de 0.876 a 0.996 y en Normal, de
0.731 a 0.047. El \acc{} con todo el conjunto rotado, llamado $A_1$, es 0.521, próximo a 0.5. Esto
se debe a que el modelo predice casi siempre la misma clase en un conjunto balanceado; no significa que
sus decisiones sean aleatorias.
...
La Tabla~\ref{tab:mc} compara las estimaciones de Monte Carlo con esta recta. Las diferencias son
menores que 0.0005 para los 18 valores de $p$, lo que funciona como una verificación independiente de la
simulación.
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.54\textwidth}\vspace{0pt}
    \centering
    \includegraphics[width=\linewidth]{expected_accuracy_vs_p.png}
  ...
  \begin{minipage}[t]{0.43\textwidth}\vspace{0pt}
    \centering
    \includegraphics[width=\linewidth]{loss_probability_vs_p.png}
...
La condición $L(p)>0.1$ equivale a obtener como máximo 329 aciertos: al menos 47 menos que sin
rotaciones. La tolerancia de 0.1 corresponde a diez puntos porcentuales de \acc{}. En los valores de la
grilla con $p\le0.20$ ninguna de las 2000 simulaciones superó esa tolerancia, pero una frecuencia cero no
implica que el evento sea imposible. Para $p=0.25$ la frecuencia estimada fue 0.0140; para $p=0.45$, 0.9715,
y desde $p=0.50$ fue próxima a uno. Por lo tanto, no hay un porcentaje de rotaciones que estos resultados
permitan declarar libre de riesgo.

La curva tiene forma de S. La media alcanza el umbral $A_0-\delta$ cerca de
$p^*=\delta/(A_0-A_1)\approx0.355$, lo cual sitúa la transición cerca de $p=0.35$. En ese punto la
probabilidad estimada es 0.4985. La probabilidad no tiene por qué ser exactamente $1/2$ cuando la media
coincide con el umbral, porque el \acc{} es discreto y su distribución no es exactamente simétrica.
La transición es estrecha porque las realizaciones se concentran alrededor de su media: en los casos
simulados, el desvío estándar es cercano o inferior a 0.015.

\subsection{Distribución del accuracy (inciso d)}

La Figura~\ref{fig:hist} muestra los histogramas del \acc{} para $p=0.1$, $0.3$, $0.5$, $0.7$ y $0.9$,
con 2000 realizaciones por valor. Como el \acc{} solo toma valores de la forma $k/468$, usamos un intervalo centrado en cada valor posible, con ancho $1/468$.
...
se desplaza hacia la izquierda al crecer $p$. El ancho teórico es máximo en $p=0.5$ y simétrico alrededor de ese
valor; los histogramas presentan fluctuaciones muestrales: el desvío es 0.009, 0.013, 0.015, 0.013 y 0.009 para $p = 0.1$, $0.3$, $0.5$, $0.7$ y $0.9$. Esto se
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{accuracy_histograms.png}
  \caption{Histogramas del \emph{accuracy} para $p=0.1,\,0.3,\,0.5,\,0.7$ y $0.9$, con 2000
           realizaciones por caso. La línea discontinua indica la media estimada.}
  \label{fig:hist}
\end{figure}
...
% =====================================================================
\section{Ejercicio 3: Ley de los Grandes Números}
\label{sec:ej3}
% =====================================================================
...
converge a $q$ con probabilidad 1. La estabilización de la Figura~\ref{fig:conv} es consistente con
ese resultado, pero una trayectoria finita no demuestra convergencia casi segura. Con $N_{MC}=2000$, la
cota da un desvío de como máximo $1/\sqrt{8000}\approx0.011$ y garantiza que
$\Prob(|\hat q_N-q|\ge0.05)\le 1/(8000\cdot 0.05^2) = 0.05$. Esta cota es conservadora porque corresponde al
peor caso, $q=0.5$, y vale para cada $p$ fijado, no como una garantía simultánea para toda la grilla.
El mismo argumento vale para el estimador de $\E[A_p]$: los $A_{p,i}$ son i.i.d. y toman
valores entre 0 y 1, por lo que su varianza es finita y su promedio converge a $\E[A_p]$. La coincidencia con
la fórmula exacta en la Tabla~\ref{tab:mc} aporta una comprobación numérica del estimador.

% =====================================================================
\section{Conclusiones}
\label{sec:concl}
En el conjunto de prueba analizado, PCA permitió reducir considerablemente la dimensión manteniendo un
desempeño comparable al de las imágenes completas. Con tres componentes se obtuvo 0.829 frente a 0.818
sin PCA, y el máximo del barrido de 1 a 300 componentes fue 0.857 con $K=5$. Conservar el 90\,\%
de la varianza requirió 121 componentes: maximizar varianza conservada y favorecer la clasificación son
criterios diferentes. La comparación es descriptiva; no demuestra un $K$ óptimo para datos nuevos ni una
causa única para las variaciones del desempeño.

El sistema con $K=2$ fue sensible a las rotaciones de $180^\circ$: con todas las imágenes rotadas el
\acc{} fue 0.521 y la mayoría de las predicciones fueron de neumonía. Monte Carlo permitió estimar la disminución del
\acc{} medio y la probabilidad de una pérdida superior a diez puntos porcentuales. La coincidencia con
las fórmulas de media y varianza respaldó la implementación. La Ley de los
Grandes Números justifica la consistencia de los estimadores; una frecuencia observada nula no garantiza
que el evento sea imposible.

Las principales dificultades fueron el tamaño de las matrices, la convergencia numérica y el costo de
repetir las simulaciones. Las abordamos usando SVD sin construir la covarianza, subiendo el límite de
iteraciones del ajuste desde las 2000 del ejemplo de la consigna hasta 20000 y dejando documentados los
siete casos ($K$ entre 218 y 224) que no convergen con ningún límite, cuyo efecto es de como máximo 6
imágenes sobre 468, y precalculando las dos predicciones posibles por imagen. También mantuvimos
la separación entre entrenamiento y prueba y distinguimos los patrones visuales de las componentes de
una interpretación causal. Estas precauciones permiten describir los resultados y sus límites sin
darles más alcance que el del experimento realizado.
...
\bibitem{leongarcia} Leon-Garcia, A. (2008). \emph{Probability, Statistics, and Random Processes for
  Electrical Engineering} (tercera edición). Pearson Prentice Hall.
